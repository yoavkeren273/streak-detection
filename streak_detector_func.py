...
def estimate_oriented_ccd_bounds(points): #point = list of tuples (x,y) I PROBABLY NEED TO USE THIS ONE AS AN THE LINES OBJECT
                                          #IT RETURNS A LIST OF FOUR TUPPLES
    ...
    def convex_hull(pts):
        ...
        lower = []
        for p in pts:
            while len(lower) >= 2 and cross(lower[-2], lower[-1], p) <= 0: #cross product is negative if the point is under the line connecting the other two?
                lower.pop()
            lower.append(p)
        upper = []
...
def find_streaks(event, bandwidth,n1,n2):
    ...
    if condition1:
        ...
        if v_max_index_0 == v_max_index_1:
